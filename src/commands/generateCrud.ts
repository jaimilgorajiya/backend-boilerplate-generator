...
export const generateCrud = async () => {
    ...
    let entityNameInput = await vscode.window.showInputBox({
        ...
        validateInput: (text: string) => {
            if (!text) return 'Entity name is required';
            if (!/^[a-zA-Z]+$/.test(text)) return 'Entity name must contain only letters';
            return null;
        }
    });
    if (!entityNameInput) return;
    ...
    try {
        ...
        const writeFileSafely = async (uri: vscode.Uri, content: string) => {
            try {
        ...
        // 4. Generate Files
        // config/db.js
        await writeFileSafely(vscode.Uri.joinPath(configPath, 'db.js'), dbTemplate);

        // models/<Name>.Models.js
        await writeFileSafely(vscode.Uri.joinPath(modelsPath, `${entityName}.Models.js`), modelTemplate(entityName, fields));

        // controllers/<Name>.Controller.js
        const controllerUri = vscode.Uri.joinPath(controllersPath, `${entityName}.Controller.js`);
        await writeFileSafely(controllerUri, controllerTemplate(entityName));

        // routes/<Name>.Routes.js
        await writeFileSafely(vscode.Uri.joinPath(routesPath, `${entityName}.Routes.js`), routeTemplate(entityName));
        ...
        try {
            ...
            await writeFileSafely(serverUri, serverTemplate(entityName));
        }
